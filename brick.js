...
class stageBrick{
	constructor(ctx, stage){
		...
		this.currentBrickComposition; //스테이지 블럭 구성 정보를 저장할 배열입니다.
	}

	init(){
		var info = stageBrickInformation[stage]; //constant.js의 값을 어떻게 들고올지에 대해 공부가 필요합니다.

		/*
			가져온 스테이지 정보에 맞게, 멤버 변수를 구성합니다. 1이면 일반 벽돌, 2이면 아이템 벽돌, 0이면 공백입니다. 
		*/
		for(var i = 0; i < len(info); i++){
			for(var j = 0; j < len(info[i]); j++){
				if(info[i][j] == 1){
					currentBrickComposition[i][j] = new Brick(this.ctx, i ,j);
				} else if (info[i][j] == 2)
				{
					currentBrickComposition[i][j] = new itemBrick(this.ctx, i ,j);
				}else{
					currentBrickComposition[i][j] = null;
				}
			}
		} 
	}
	...
	drawStageBrick(){
		...
		for(var i = 0; i < len(currentBrickComposition); i++){
			for(var j = 0; j < len(currentBrickComposition[i]); j++){
				// null인 경우, 이미 충돌된 경우 continue;
				if((currentBrickComposition[i][j] != null) && (currentBrickComposition[i][j].isAlive == true)){
					currentBrickComposition[i][j].drawBrick(); // itemBrick클래스는 Brick을 상속합니다. 
				}
			}
		}
	}
